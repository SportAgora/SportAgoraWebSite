document.addEventListener("DOMContentLoaded", function() {
  ...
  // Preview da imagem -- comentei pq na pagina nao tem essas coisas, ai faz o resto do codigo nao rodar
...
if (cepInput) {
  ...
  document.querySelector('#ingresso-tabela').addEventListener('click', (e) => {
   ...
   if (typeof ingressos !== 'undefined' && Array.isArray(ingressos)) {
    ...
    ingressos.forEach(i => {
      adicionaIngresso(i.ingresso_nome, parseFloat(i.ingresso_valor), i.ingresso_quantidade, i.ingresso_meia == 1);
    });
  }

  function adicionaIngresso(nome, valor, quantidade, meiaEntrada) {
  const tabela = document.querySelector('#ingresso-tabela tbody');
  const novaLinha = document.createElement('tr');
  let valorMeia = meiaEntrada ? (valor / 2).toFixed(2) : '-';
  const ingressoId = Date.now() + Math.floor(Math.random() * 1000); // garante id único
  ...
  form.insertAdjacentHTML('beforeend', `
    ...
    <input type="hidden" data-id="${ingressoId}" name="ingressos[valor][]" value="${valor}">
    <input type="hidden" data-id="${ingressoId}" name="ingressos[quantidade][]" value="${quantidade}">
    <input type="hidden" data-id="${ingressoId}" name="ingressos[meia][]" value="${meiaEntrada}">
  `);
}
